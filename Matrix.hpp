...
protected:
    ...
    static constexpr long double MINIMUM_VALUE{ 0.0 };
...
public:
     ...
     */
    explicit Matrix( const unsigned long square_size ) : col_count( square_size ), row_count( square_size ) {
        if ( square_size <= 0 ) {
     ...
     */
    Matrix( unsigned long cols, unsigned long rows );
     ...
     */
    void set_value( unsigned long col, unsigned long row, double val );
     ...
     */
    double get_value( unsigned long col, unsigned long row ) const;
     ...
     */
    unsigned long get_col_count() const { return col_count; };

    /**
     * Get the number of rows.
     * @return the number of rows
     */
    unsigned long get_row_count() const { return row_count; };
